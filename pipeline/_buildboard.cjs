...
const GUIDE={clinician:{label:'치료사용',subs:[{slug:'dosu-clinician',label:'도수치료'},{slug:'eswt-clinician',label:'체외충격파'},{slug:'dosu-faq',label:'채팅 FAQ'}]},patient:{label:'환자용',subs:[{slug:'dosu-patient',label:'도수치료'},{slug:'eswt-patient',label:'체외충격파'}]}};Object.keys(GUIDE).forEach(function(k){GUIDE[k].subs.forEach(function(sb){try{var o=JSON.parse(fs.readFileSync('../guide/'+sb.slug+'.sections.json','utf8'));sb.title=o.title;sb.sections=o.sections;}catch(e){sb.title='';sb.sections=[];}});});
const DATA={ko,press:press.map(function(r){var c=Object.assign({},r);delete c.desc;return c;}),insure:insure.map(function(r){var c=Object.assign({},r);delete c.desc;return c;}),stmt,petitions,notices,rallyBoard,subBoard,researchBoard,busBoard,busInfo,buzz:(BUZZON?buzz:{}),guide:GUIDE,perfcard:PERFCARD,rally:'2026-06-28',deadline:'2026-07-04',upd:(CO.updated||'6.22 11시 기준')};
...
const ST={w:window.innerWidth,expanded:null,sort:{ko:'new',press:'new',stmt:'old',insure:'new'},noticesOpen:false,noticesSort:'new',ncard:0,lb:null,pop:true,feed:'ko',grp:null,actHidden:false,boardPop:null,route:false,routeFrom:'경복궁역',routeFull:false,acc:null,busPop:true,buzz:false,guide:false,guideMode:'clinician',guideSub:0,guideView:'cards',guideSec:0,buzzKw:'',buzzPeriod:'3개월',buzzMetric:'cnt',buzzSentCh:'community',buzzCntCh:'all',buzzView:'all4',buzzZoom:1,buzzPanelPer:'3개월'};
...
function guideOverlay(inline){
  if(!ST.guide) return '';
  var G=DATA.guide||{};var mode=(ST.guideMode==="patient")?"patient":"clinician";var cat=G[mode];
  if(!cat||!(cat.subs&&cat.subs.length)) return '<div style="flex:1;display:flex;align-items:center;justify-content:center;color:#5C677D;font-size:13px;padding:24px;">정리본 준비 중</div>';
  var subs=cat.subs;var si=Math.max(0,Math.min(subs.length-1,+ST.guideSub||0));var sub=subs[si]||{sections:[],title:""};
  var secs=sub.sections||[];var doc=ST.guideView==="doc";
  function tb(i,label,on){return '<button data-act="guidesub" data-i="'+i+'" style="flex:1;min-width:0;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;cursor:pointer;font-weight:800;font-size:13px;padding:7px 4px;border-radius:8px;border:1px solid '+(on?"#46C088":"rgba(255,255,255,.12)")+';background:'+(on?"#46C088":"rgba(255,255,255,.04)")+';color:'+(on?"#06140C":"rgba(255,255,255,.6)")+';">'+E(label)+'</button>';}
  var subTabs=subs.map(function(sb,i){return tb(i,sb.label,i===si);}).join("");
  function vb(v,label){var on=(doc?"doc":"cards")===v;return '<button data-act="guideview" data-v="'+v+'" style="flex:1;cursor:pointer;font-size:12px;font-weight:800;padding:6px 4px;border-radius:7px;border:1px solid '+(on?"rgba(70,192,136,.6)":"rgba(255,255,255,.12)")+';background:'+(on?"rgba(70,192,136,.16)":"transparent")+';color:'+(on?"#7FE6B0":"rgba(255,255,255,.55)")+';">'+label+'</button>';}
  var viewBtns=vb("cards","섹션별 보기")+vb("doc","한번에 보기");
  var badge='<span style="flex:none;font-size:12px;font-weight:800;color:#7FE6B0;background:rgba(70,192,136,.14);border:1px solid rgba(70,192,136,.4);padding:6px 11px;border-radius:8px;white-space:nowrap;">'+E(cat.label)+'</span>';
  var STY="<style>.gdoc{color:#D7DEEC;font-size:13.5px;line-height:1.78}.gdoc h1{font-size:19px;color:#F4F6FB;border-bottom:2px solid #46C088;padding-bottom:8px;margin:4px 0 14px;line-height:1.4}.gdoc h2{font-size:15.5px;color:#7FB0E8;border-left:4px solid #46C088;background:rgba(70,192,136,.08);padding:7px 0 7px 11px;margin:20px 0 10px}.gdoc h3{font-size:14px;color:#F4F6FB;font-weight:800;margin:16px 0 6px}.gdoc h4{font-size:13px;color:#C2CAD8;margin:12px 0 5px}.gdoc p{margin:8px 0}.gdoc ul,.gdoc ol{margin:8px 0 12px 1.3em;padding:0}.gdoc li{margin:6px 0}.gdoc li.sub{list-style:circle;color:#AEB6C6}.gdoc p.src{color:#9AA8BE;font-size:12.5px;background:rgba(255,255,255,.04);border-left:3px solid rgba(127,176,232,.5);padding:6px 11px;margin:5px 0 10px;border-radius:0 4px 4px 0}.gdoc table{border-collapse:collapse;width:100%;margin:12px 0;font-size:13px}.gdoc th,.gdoc td{border:1px solid rgba(255,255,255,.14);padding:7px 9px;text-align:left;vertical-align:top}.gdoc th{background:rgba(91,134,200,.18)}.gdoc tr:nth-child(even) td{background:rgba(255,255,255,.02)}.gdoc pre{background:#0F1420;border:1px solid rgba(255,255,255,.1);border-radius:6px;padding:12px;white-space:pre-wrap;font-size:12.5px;color:#C2CAD8;overflow-x:auto}.gdoc hr{border:none;border-top:1px solid rgba(255,255,255,.1);margin:16px 0}</style>";
  var body;
  if(doc){body='<div class="gdoc">'+(sub.title?'<h1>'+E(sub.title)+'</h1>':"")+secs.map(function(x){return '<h2>'+E(x.h)+'</h2>'+x.html;}).join("")+'</div>';}
  else{var op=(typeof ST.guideSec==="number")?ST.guideSec:0;body=secs.map(function(x,i){var o=op===i;return '<div style="border:1px solid rgba(255,255,255,.1);border-radius:12px;margin-bottom:10px;overflow:hidden;background:#131A26;"><button data-act="guidesec" data-i="'+i+'" style="width:100%;text-align:left;cursor:pointer;display:flex;align-items:center;gap:8px;padding:12px 14px;background:'+(o?"rgba(70,192,136,.12)":"transparent")+';border:none;color:#F4F6FB;font-size:14px;font-weight:800;"><span style="flex:1;min-width:0;">'+E(x.h)+'</span><span style="flex:none;color:#7FE6B0;font-size:12px;">'+(o?"▲":"▼")+'</span></button>'+(o?'<div class="gdoc" style="padding:2px 16px 16px;">'+x.html+'</div>':"")+'</div>';}).join("");}
  return '<div class="sb" style="flex:1;min-width:0;min-height:0;overflow-y:auto;display:flex;flex-direction:column;border-radius:16px;border:1px solid rgba(255,255,255,.1);background:radial-gradient(125% 80% at 50% 0%,#16241C,#0D1118);box-shadow:0 10px 28px rgba(0,0,0,.3);">'+'<div style="position:sticky;top:0;z-index:5;padding:10px 14px;background:#16241C;border-bottom:1px solid rgba(255,255,255,.08);border-radius:16px 16px 0 0;"><div style="display:flex;align-items:center;gap:7px;">'+badge+'<span style="flex:1;display:flex;gap:6px;">'+subTabs+'</span><button data-act="guidex" style="flex:none;border:1px solid rgba(255,255,255,.2);background:rgba(255,255,255,.08);color:#fff;font-size:13px;font-weight:800;padding:6px 12px;border-radius:8px;cursor:pointer;">닫기 ✕</button></div><div style="display:flex;gap:6px;margin-top:8px;">'+viewBtns+'</div></div>'+STY+'<div style="padding:14px 16px 30px;">'+body+'</div>'+'</div>';
}
function buzzOverlay(inline){
  ...
  if(metric==='cnt'){
    ...
    if(per==='1일'){
      chartInner='<div style="color:#8A93A6;font-size:13px;padding:34px 16px;text-align:center;line-height:1.6;">구글·네이버 검색 지수는 일(日) 단위 상대지수예요.<br>1일(시간대별) 보기는 지원하지 않아요. 1주일 이상을 선택해 주세요.</div>';
      unit='지수';
    } else {
      var srcD,srcV;
...
function render(){
  ...
  '<div style="background:#0E121B;color:#E9EBF1;height:'+(wide?'100vh':'auto')+';overflow:'+(wide?'hidden':'visible')+';display:flex;flex-direction:column;">'+
   ...
   '<main style="flex:1;min-height:0;display:grid;grid-template-columns:'+mainCols+';gap:14px;padding:14px;">'+
    ...
    (ST.buzz?('<div style="min-height:0;display:flex;flex-direction:column;">'+buzzOverlay(true)+'</div>'):ST.guide?('<div style="min-height:0;display:flex;flex-direction:column;">'+guideOverlay(true)+'</div>'):('<div style="min-height:0;display:grid;grid-template-columns:minmax(0,1fr);grid-template-rows:'+rowRows+';gap:14px;">'+
     bigFeed()+
...
document.addEventListener('keydown',e=>{if(e.key!=='Escape')return;if(ST.buzz){ST.buzz=false;render();return;}if(ST.guide){ST.guide=false;render();return;}if(ST.route){ST.route=false;ST.routeFull=false;render();return;}if(ST.boardPop){ST.boardPop=null;render();return;}if(ST.grp){ST.grp=null;render();}else if(ST.lb!=null){ST.lb=null;render();}else if(ST.pop){ST.pop=false;render();}else if(ST.busPop){ST.busPop=false;render();}});
