...
function newsHTML(){
 var list=filtered();var nso=(state.sort&&state.sort.news)||'new';list.sort(function(a,b){return nso==='old'?String(a.dt).localeCompare(String(b.dt)):String(b.dt).localeCompare(String(a.dt));});var _pins=[],_others=[];list.forEach(function(a){(a.pin?_pins:_others).push(a);});list=_pins.concat(_others);var lead=list[0],rest=list.slice(1);
...
(function(){var K='ptnews_surveypop';try{if(sessionStorage.getItem(K))return;}catch(_){}var LS='display:block;padding:11px 14px;border-radius:10px;background:#1E2840;border:1px solid rgba(127,199,240,.4);color:#CFE3F5;font-size:14px;font-weight:700;text-decoration:none;text-align:center;';var LINKS='<div style="display:flex;flex-direction:column;gap:8px;margin-top:14px;width:100%;max-width:440px;"><a href="https://naver.me/Gpr2EXFj" target="_blank" rel="noopener" style="'+LS+'">집회 참여 조사 (1분)</a><a href="https://naver.me/IgMhpn5i" target="_blank" rel="noopener" style="'+LS+'">치료사 고용 피해 설문 (5분)</a><a href="https://naver.me/xF4Ex3F5" target="_blank" rel="noopener" style="'+LS+'">환자 피해 설문 (2분)</a><a href="https://cafe.naver.com/onpta" target="_blank" rel="noopener" style="'+LS+'background:#0F3D3A;border-color:rgba(95,227,168,.5);color:#9FE9C8;">카페 가입 (30초)</a></div>';var P=document.createElement('div');P.id='surveyPop';P.style.cssText='position:fixed;right:14px;bottom:14px;z-index:190;width:190px;max-width:44vw;background:#161B28;border:1px solid rgba(255,255,255,.18);border-radius:14px;box-shadow:0 16px 40px rgba(0,0,0,.55);overflow:hidden;';P.innerHTML='<div style="display:flex;align-items:center;gap:6px;padding:8px 10px;background:linear-gradient(180deg,#1B2438,#141B29);"><span style="font-size:12px;font-weight:900;color:#5FE3A8;letter-spacing:-.02em;">📋 설문·참여</span><span style="flex:1;"></span><button id="surveyPopX" aria-label="닫기" style="border:none;background:rgba(255,255,255,.1);color:#fff;font-size:12px;font-weight:800;width:22px;height:22px;border-radius:6px;cursor:pointer;line-height:1;padding:0;">✕</button></div><img id="surveyPopImg" src="img/poster_survey4.png" alt="도수치료 관리급여 대응 설문·참여 포스터" loading="lazy" style="display:block;width:100%;height:auto;cursor:zoom-in;"><div style="padding:7px 10px 9px;font-size:10.5px;color:#9AA8BE;font-weight:700;text-align:center;letter-spacing:-.01em;">탭하면 크게 · QR·링크로 참여</div>';document.body.appendChild(P);P.querySelector('#surveyPopX').onclick=function(){P.remove();try{sessionStorage.setItem(K,'1');}catch(_){}};function openBig(){var ov=document.createElement('div');ov.style.cssText='position:fixed;inset:0;z-index:340;background:rgba(9,11,17,.92);display:flex;align-items:flex-start;justify-content:center;padding:20px 14px;overflow:auto;';ov.innerHTML='<div style="display:flex;flex-direction:column;align-items:center;max-width:96vw;"><img src="img/poster_survey4.png" alt="설문·참여 포스터" style="max-width:min(560px,92vw);width:100%;height:auto;border-radius:8px;box-shadow:0 14px 50px rgba(0,0,0,.7);">'+LINKS+'<button id="surveyBigX" style="margin-top:16px;border:1px solid rgba(255,255,255,.25);background:rgba(255,255,255,.08);color:#fff;font-size:13px;font-weight:800;padding:9px 20px;border-radius:9px;cursor:pointer;">닫기 ✕</button></div>';ov.onclick=function(e){if(e.target===ov||e.target.id==='surveyBigX')ov.remove();};document.body.appendChild(ov);}P.querySelector('#surveyPopImg').onclick=openBig;})();
})();`;
